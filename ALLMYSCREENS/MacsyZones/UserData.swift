...
class UserLayouts: UserData, ObservableObject {
    ...
    var portraitHalvesLayout: UserLayout {
        .init(name: "Portrait Halves", sectionConfigs: QuickLayoutPresets.sectionConfigs(for: .halves))
    }

    var portraitThirdsLayout: UserLayout {
        .init(name: "Portrait Thirds", sectionConfigs: QuickLayoutPresets.sectionConfigs(for: .thirds))
    }
    ...
    override func load() {
        ...
        do {
            ...
            if layouts.isEmpty {
                layouts["Portrait Halves"] = portraitHalvesLayout
                layouts["Portrait Thirds"] = portraitThirdsLayout
                ...
                currentLayoutName = "Portrait Halves"
            } else {
    ...
    func upsertZoneLayout(name: String, sectionConfigs: [SectionConfig]) {
        stopEditing()

        if let existing = layouts[name] {
            existing.hideAllWindows()
            existing.layoutWindow.closeAllWindows()
            layouts.removeValue(forKey: name)
        }

        layouts[name] = UserLayout(name: name, sectionConfigs: sectionConfigs)
        save()
    }
