    ...
    var body: some View {
        VStack(alignment: .center, spacing: 8) {
            HStack(alignment: .center, spacing: 5) {
                if proLock.isPro {
                    Text("ALLMYSCREENS Pro").font(.headline)
                } else {
                    Text("ALLMYSCREENS").font(.headline)
                }
                Button(action: {
                    resetDialogs()
                    showDialog = true
        ...
        .alert(isPresented: $showDialog) {
            if showResetToDefaultsDialog {
               ...
               return Alert(
                ...
                return Alert(
                    title: Text("About ALLMYSCREENS"),
                    message: Text("""
                        ALLMYSCREENS — portrait monitor zone containers.

                        Fork of MacsyZones (GPL-3.0) by Oğuzhan Eroğlu.
        ...
        if !ready.isReady {
            VStack {
                VStack(alignment: .center) {
                    Text("ALLMYSCREENS is loading...").padding(.bottom, 10).padding(.top, 25)
